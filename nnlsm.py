...
def normal_eq_comb(AtA, AtB, PassSet=None):
    ...
    num_cholesky = 0
    num_eq = 0
    # print("############ type(PassSet) ",type(PassSet))
    # print("########### PassSet.all() ", PassSet.all())
    if AtB.size == 0:
        Z = np.zeros([])
...
#        A = np.random.rand(m, n)
#        A = sla.hankel(np.random.randn(m),np.random.randn(n))
        Ag = np.random.rand(m+n)
        Ag[np.random.rand(m+n) < 0.99 ] = 0 # < 0.5] = 0
        A = sla.toeplitz(Ag[:m],Ag[m:])
        X_org = np.random.rand(n, k)
        X_org[np.random.rand(n, k) < 0.99 ] = 0 # < 0.5] = 0
        print (X_org.sum())
        B = A.dot(X_org)
...
#        if rel_norm1 > 10e-5 or rel_norm2  > 10e-5 or rel_norm3 > 10e-5:
        if rel_norm1 > 10e-5  or rel_norm3 > 10e-5:
            break
        print ('')
